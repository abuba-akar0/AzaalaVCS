    ...
    private void performDiff() {
        ...
        com.azaala.vcs.async.ProgressListener progressListener = new com.azaala.vcs.async.ProgressListener() {
            ...
            public void onProgress(String message, int progress) {
                statsLabel.setText(message + " (" + progress + "%)");
            }

            @Override
            public void onSuccess(String result) {
                // Diff completed, display result
                displayDiffResults(commit1IdPrefix, commit2IdPrefix);
            }

            @Override
            public void onError(String message, Throwable exception) {
                diffArea.setText("ERROR: " + message);
                statsLabel.setText("Error: " + message);
            }
        };

        // Execute async worker with database enhancement
        statsLabel.setText("Comparing commits...");
        diffArea.setText("Loading diff...\n\nComparing commits with database enhancement...");

        com.azaala.vcs.async.DiffWorker worker =
            new com.azaala.vcs.async.DiffWorker(vcs, repository, commit1IdPrefix, commit2IdPrefix, progressListener);
        worker.execute();
    }

    private void displayDiffResults(String commit1IdPrefix, String commit2IdPrefix) {
        try {
            // Find commits by ID
            Commit commit1 = null;
            Commit commit2 = null;
